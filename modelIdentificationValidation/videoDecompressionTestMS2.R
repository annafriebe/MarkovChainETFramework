...
EstimateValidateModel <- function(index, dataFrame, dfTestList, outputDir, maxNStates, startStdDev, nPartitions){
  ...
  for (i in 1:nTest){
    dfTest <- dfTestList[[i]]
    modViterbi <- depmixS4::depmix(outputs~1, data=dfTest, nstates=tree$leafCount, family = stats::gaussian())
    modViterbi <- depmixS4::setpars(modViterbi, depmixS4::getpars(fittedMod))
    viterbiStateProbs <- depmixS4::viterbi(modViterbi)

    zObs <- lib$dataConsistencyModelValidation$logLikelihoodStatesTraj(dfTest, modViterbi, transitionMatrix, normalParams)
    TObs <- list()
    for(j in 1:(nStates+1)){
  ...
  modViterbi <- depmixS4::depmix(outputs~1, data=dataFrame, nstates=tree$leafCount, family = stats::gaussian())
  modViterbi <- depmixS4::setpars(modViterbi, depmixS4::getpars(fittedMod))
  viterbiStateProbs <- depmixS4::viterbi(modViterbi)

  zObs <- lib$dataConsistencyModelValidation$logLikelihoodStatesTraj(dataFrame, modViterbi, transitionMatrix, normalParams)
  TObs <- list()
  for(j in 1:(nStates+1)){
    TObs[[j]] <- mean((zObs[,j] - Ez[[j]])^2/Vz[[j]])
  }
...
dataFrame1 <- read.csv("input/videoStateTimesTestMS2_other.csv")
dataFrame1 <- lib$importData$AdaptDataFrame(dataFrame1, 1, TRUE)

dataFrame2 <- read.csv("input/videoStateTimesTestMS2.csv")
...
MP <- 100
M <- 100
outputDir1 <- "output/videoDecompressionMS2_other/"
outputDir2 <- "output/videoDecompressionMS2/"
